...
public class 계산기3_후위표기법 {
	...
	static int priority(char input) {
		...
		case '*': return 1;
		case '/': return 1;
		default: return 3; // 값이 더 작은 '(' 이랑 비교할때 pop안되도록
		}
	}
	
	static int calc(int x, int y, char oper) {
		switch (oper) {
		case '+': return x + y;
		case '-': return x - y;
		case '*': return x * y;
...
//		System.out.println(sb.toString());
	...
	static long calc_postfix() {
		Stack<Integer> num = new Stack<Integer>();
		int sum =0;
		for(int i =0; i< ans.size();i++) {
			if('0' < ans.get(i) && ans.get(i) <= '9')
				num.push(ans.get(i) - '0');
			else {
				int x = num.pop();
				int y = num.pop();
				sum = calc(x,y,ans.get(i));
				num.push(sum);
			}
		}
		return sum;
